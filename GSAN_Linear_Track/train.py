...
#         x = self.bn(x)
        ...
        encoded_outputs,last_encoded_output = self.gru(encoded_outputs,h)
        encoded_outputs = encoded_outputs.view(batchsize,vnum,seqlen,self.config.hidden_size)
        attention_mask = attention_mask.view(batchsize,seqlen,vnum)
        attention_mask = torch.transpose(attention_mask,1,2)
        attention_mask = attention_mask!=-10000.0
        return encoded_outputs,last_encoded_output,attention[0].data.cpu().numpy(),attention_mask

class GSAN_linear(nn.Module):
    def __init__(self,config):
        super(GSAN_linear, self).__init__()
        self.gsan = Encoder(config)
        self.linear = nn.Linear(config.hidden_size,2)
    def forward(self,x,h=None):
        encoded_outputs,last_encoded_output,attention,attention_mask = self.gsan(x,h)
        y = self.linear(encoded_outputs)
        y *= attention_mask.unsqueeze(-1)
        return y + x,last_encoded_output
...
def val(val_dataloader, model, device):
    ...
    for i,x in enumerate(val_dataloader):
        ...
        x_t = x_t[:,:,-1,:].unsqueeze(2)
        pred_track_t = []        
        for time_step in range(pred_len): # 10
            x_t, h_t = model(x_t.float().to(device),h_t)
            h_t = h_t.data
            pred_track_t.append(x_t[:,0,:,:].data.cpu().numpy()) #10*(256,1,2)
        pred_track.append(np.concatenate(pred_track_t,axis=1))
        ture_track.append(x[:,0,hist_len:,:].data.cpu().numpy()) #m*(256,10,2)
...
if __name__ == "__main__":    
    ...
    model = GSAN_linear(config).to(device)
    if PRE_TRAIN:
