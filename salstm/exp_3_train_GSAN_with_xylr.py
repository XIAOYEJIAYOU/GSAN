...
def fit(x,encoder,decoder,device):
    encoderInput = x[:,:,:hist_len,:2].float().to(device)
    encoderOutput,lastEncoderHiddenState,attention = encoder(encoderInput)
    decoderInput = lastEncoderHiddenState.squeeze(0)
    mask = x[:,0,-1,2:].float().to(device) # keep, right, left
    decoderOutput = decoder(decoderInput,mask)
...
if __name__ == "__main__":
    ...
    # 3.5 add left, right lane existed or not mask
    # 5_dim: x, y, keep_exist, left_exist, right_exist
    # But Softmax keep the [keep,right,left] order
    # So change the order
    X_train = np.concatenate((X_train[:,:,:,0:2],np.ones((X_train.shape[0],X_train.shape[1],X_train.shape[2],1)),np.expand_dims(X_train[:,:,:,3],axis=3) ,np.expand_dims(X_train[:,:,:,2],axis=3)),axis=3)
    X_eval = np.concatenate((X_eval[:,:,:,0:2],np.ones((X_eval.shape[0],X_eval.shape[1],X_eval.shape[2],1)),np.expand_dims(X_eval[:,:,:,3],axis=3) ,np.expand_dims(X_eval[:,:,:,2],axis=3)),axis=3)
    X_test = np.concatenate((X_test[:,:,:,0:2],np.ones((X_test.shape[0],X_test.shape[1],X_test.shape[2],1)),np.expand_dims(X_test[:,:,:,3],axis=3) ,np.expand_dims(X_test[:,:,:,2],axis=3)),axis=3)
    ...
    if SAVE_PATH:
        ...
        torch.save(encoder.state_dict(), decoder_path)
